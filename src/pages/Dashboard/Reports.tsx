...
const Reports = () => {
  ...
  return (
    <Layout>
      <h1 className="text-xl font-semibold text-ink-800 dark:text-ink-100 mb-1">Reports</h1>
      <p className="text-ink-400 dark:text-ink-500 text-sm mb-6">Ticket breakdown and agent performance.</p>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div className="bg-white dark:bg-ink-800 rounded-lg shadow-card border border-ink-100 dark:border-ink-700 p-6">
          <h3 className="font-semibold text-ink-800 dark:text-ink-100 mb-4">Tickets by Status</h3>
          <div className="space-y-3">
            {byStatus.map((s) => (
              <div key={s.status} className="flex items-center gap-3">
                <div className="w-28 shrink-0"><Badge label={s.status} className={statusColors[s.status]} /></div>
                <div className="flex-1 bg-ink-50 dark:bg-ink-700/40 rounded-full h-3 overflow-hidden">
                  <div
                    className="bg-primary-500 h-full rounded-full"
                    style={{ width: `${(s.count / maxStatus) * 100}%` }}
                  />
                </div>
                <span className="text-sm font-medium text-ink-600 dark:text-ink-300 w-6 text-right">{s.count}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-ink-800 rounded-lg shadow-card border border-ink-100 dark:border-ink-700 p-6">
          <h3 className="font-semibold text-ink-800 dark:text-ink-100 mb-4">Tickets by Priority</h3>
          <div className="grid grid-cols-2 gap-4">
            {byPriority.map((p) => (
              <div key={p.priority} className="border border-ink-100 dark:border-ink-700 rounded-lg p-4 text-center">
                <Badge label={p.priority} className={priorityColors[p.priority]} />
                <p className="text-2xl font-bold text-ink-800 dark:text-ink-100 mt-2">{p.count}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-ink-800 rounded-lg shadow-card border border-ink-100 dark:border-ink-700 p-6">
        <h3 className="font-semibold text-ink-800 dark:text-ink-100 mb-4">Agent Performance</h3>
        {byAgent.length === 0 ? (
          <p className="text-sm text-ink-400 dark:text-ink-500">No support agents yet.</p>
        ) : (
          <table className="w-full text-sm">
            <thead className="text-ink-400 dark:text-ink-500 text-xs uppercase">
              <tr>
            ...
            <tbody className="divide-y divide-ink-100 dark:divide-ink-700">
              {byAgent.map((a) => (
                <tr key={a.agent}>
                  <td className="py-2.5 font-medium text-ink-700 dark:text-ink-200">{a.agent}</td>
